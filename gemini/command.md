...
GoalSettingForm에서 문제가 발생했습니다. 아래에 나열된 문제를 해결해주세요.
1. 체중 입력 값이 음수가 될 수 있습니다. 
2. 입력 값이 적절하지 않은 값이면 goalValidators에서 나오는 에러 메시지가 표시되야 하는데 표시되지 않습니다.

GoalSettingForm.test.tsx와 goalValidators.test.ts를 기반으로, 모든 테스트가 정상적으로 시행될 수 있도록 GoalSettingForm 코드를 수정해주세요.
