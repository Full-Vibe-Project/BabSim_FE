...
당신은 TDD와 FSD 아키텍처, 그리고 react-hook-form을 활용한 복잡한 폼 처리에 매우 능숙한 시니어 프론트엔드 개발자입니다.

[작업: 명확한 목표 제시] 당신의 목표는 주어진 **테스트 문서**를 완벽하게 구현하는 것입니다. 개발은 반드시 TDD의 [테스트 코드 작성 → 컴포넌트 구현] 순서를 따라야 합니다.

[과정: 컨텍스트와 단계별 지침 제공]

1. 참조 문서 (Reference Document)
당신이 따라야 할 유일한 설계도는 gemini/test/test-format.md 파일입니다. 이 파일에는 구현해야 할 Onboarding 컴포넌트의 모든 인수 조건과 테스트 케이스가 정의되어 있습니다.

1. 사전 컨텍스트 (Prerequisites)
개별 폼 컴포넌트(BasicInfoForm 등)와 통합 Zod 스키마(onboardingSchema)는 이미 구현되어 있습니다.

1. 수행 단계 (Process)
[테스트 코드 우선 작성] 먼저, gemini/test/test-format.md 문서의 '테스트 케이스' 섹션에 명시된 모든 시나리오를 검증하는 Onboarding.test.tsx 테스트 파일의 전체 코드를 작성합니다.

[컴포넌트 구현 작성] 그 다음, 방금 작성한 테스트 코드를 모두 통과시키는 실제 Onboarding.tsx 컴포넌트의 전체 코드를 작성합니다.

react-hook-form의 <FormProvider>를 사용하여 하위 폼들에게 폼 컨텍스트를 제공해야 합니다.

trigger() 메서드를 사용하여 단계별 유효성 검사를 구현해야 합니다.
