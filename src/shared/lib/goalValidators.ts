...
export const validateWeeklyGoal = (weeklyGoal: number, maxLimit: number): { isSuccess: boolean; msg: string | null } => {
    if (weeklyGoal < 0) {
        return { isSuccess: false, msg: '주간 목표는 0 이상의 값이어야 합니다.' };
    }
    if (!Number.isInteger(weeklyGoal)) {
        return { isSuccess: false, msg: '주간 목표는 정수만 입력 가능합니다.' };
    }
    if (weeklyGoal > maxLimit) {
