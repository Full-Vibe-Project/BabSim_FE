...
const baseSchema = z.object({
  ...
  // New fields from goal-setting.html
  goalPeriod: z.string().min(1, '목표 기간을 설정해주세요.').optional(),
  exerciseCount: z.number().min(0, '주간 운동 횟수는 0 이상이어야 합니다.').optional(),
  dailyWaterIntake: z.number().min(0, '하루 물 섭취량은 0 이상이어야 합니다.').optional(),
  dailySteps: z.number().min(0, '하루 걸음 수는 0 이상이어야 합니다.').optional(),
});
